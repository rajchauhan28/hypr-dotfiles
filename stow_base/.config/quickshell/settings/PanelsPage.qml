...
ColumnLayout {
    ...
    // The left sidebar's geometry used to be duplicated here as well. Two cards
    // editing the same keys meant one card's "reset" link appeared while the
    // other still read as clean, so it lives only on the Sidebar page now.
}
